...
# 2. DESIGN DU POWERPOINT
def appliquer_style_perso(slide, titre_texte, est_correction=False):
    couleur_fond = RGBColor(200, 0, 0) if est_correction else RGBColor(0, 82, 204)
...
def ajouter_slide_texte(prs, titre, paragraphes, est_correction=False, prompt_img=None):
    for i in range(0, len(paragraphes), 4):
        ...
        if i == 0 and prompt_img and not est_correction:
            ...
            url = f"https://image.pollinations.ai/prompt/professional_illustration_{prompt_img.replace(' ', '_')}?width=512&height=512&nologo=true&seed={seed}"
            try:
        ...
        for p_text in paragraphes[i:i+4]:
            ...
            p.font.size, p.font.color.rgb = Pt(16), RGBColor(40, 40, 40)
            p.space_after = Pt(10)

def generer_pptx_v10(diplome, sujet, cours_txt, formateur_txt):
    prs = Presentation()
...
with col1:
    ...
    sujet = st.text_input("Sujet de la leçon :", placeholder="ex: Analyse du système de freinage")
with col2:
    # AJOUT DE LA POSSIBILITÉ DE CHANGER LE LIEU
    lieu = st.text_input("Lieu du scénario :", value="Chartres")

if st.button("🚀 GÉNÉRER LE PACK COMPLET"):
    if sujet:
        with st.spinner("Conception pédagogique en cours..."):
            ...
            prompt = f"""
            Tu es un expert en pédagogie. Crée un cours de 60min pour {diplome} sur {sujet}.
            Respecte le référentiel {diplome}. Le ton est ludique, avec de l'humour, et le scénario se déroule à {lieu} ou ses alentours.
            
            PARTIE APPRENTI (SANS RÉPONSES) :
            ### 1. L'Accroche [IMG: colorful illustration of {sujet}]
            ### 2. Référentiel (Compétences ciblées)
            ### 3. Mission Professionnelle (Contexte à {lieu}) [IMG: photo of {sujet}]
            ### 4. Exercice d'Application (Énoncé détaillé)
            ### 5. Quiz QCM (3 questions)
            ### 6. Le Vrai ou Faux (5 points)
            ### 7. Travail en Autonomie (60 min)
            ### 8. La Synthèse (L'essentiel)
            ...
            ### 3. Conseils d'animation pour le niveau {diplome}
            """
            ...
            file = generer_pptx_v10(diplome, sujet, cours_txt, formateur_txt)
